(function () {
  ...
  function statusBadge(st) {
    ...
    const label = { confirmed: "CONFIRMADO", hold: "ESPERA SEÑA", cancelled: "CANCELADO", expired: "VENCIDO" };
  ...
  function render() {
    ...
    const box = $("#dayList");
    if (!day.length) {
      box.innerHTML = "<div class='kicker'>" + date + "</div><p class='lead'>No hay turnos este día.</p>";
    } else {
      box.innerHTML = "<div class='kicker'>Turnos del " + E.formatDateHuman(date) + "</div>";
      day.forEach((a) => {
        ...
        el.className = "appt";
        el.innerHTML = `
          <div><b>${a.time}</b><div class="meta">${a.duration} min</div></div>
          <div>
    ...
    box.onclick = (ev) => {
      ...
      if (pay) { E.confirmPayment(pay, "manual"); toast("Seña marcada"); render(); }
      if (cancel) { E.cancelAppointment(cancel, "admin"); toast("Turno cancelado"); render(); }
    };

    const rows = [...state.appointments].sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time));
    $("#allTable").innerHTML = "<tr><th>Fecha</th><th>Hora</th><th>Cliente</th><th>Servicio</th><th>Pro</th><th>Seña</th><th>Estado</th></tr>" +
      rows.map((a) => {
        const s = E.getService(a.serviceId);
        const p = E.getPro(a.professionalId);
        return `<tr>
          <td>${a.date}</td><td>${a.time}</td><td>${a.clientName}<br><span class="meta">${a.clientPhone}</span></td>
          <td>${s ? s.name : ""}</td><td>${p ? p.name : ""}</td>
  ...
  $("#btnRemindDay").onclick = () => {
    const date = $("#adminDate").value;
    const state = E.loadState();
    const list = state.appointments.filter((a) => a.date === date && a.status === "confirmed");
    if (!list.length) { toast("No hay turnos confirmados ese día"); return; }
    const first = list[0];
    ...
    if (list.length > 1) toast("Se abrió el primero. Repetí WhatsApp en cada turno de la lista.");
  };
  ...
  $("#fileImport").onchange = (e) => {
    ...
    reader.onload = () => {
      try { E.importState(reader.result); toast("Datos importados"); render(); }
